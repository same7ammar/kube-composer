...
import { Plus, X, FileText, Trash2, AlertTriangle } from 'lucide-react';
import type { ConfigMap } from '../types';

interface ConfigMapManagerProps {
  configMaps: ConfigMap[];
  namespaces: string[];
  onAddConfigMap: (configMap: ConfigMap) => void;
  onDeleteConfigMap: (configMapName: string) => void;
  onClose: () => void;
}

export function ConfigMapManager({ 
  configMaps, 
  namespaces,
  onAddConfigMap, 
  onDeleteConfigMap, 
  onClose 
}: ConfigMapManagerProps) {
  ...
  const validateConfigMapName = (name: string): string[] => {
    const errors: string[] = [];
    
    if (!name) {
      errors.push('ConfigMap name is required');
      return errors;
    }
    ...
    if (configMaps.some(cm => cm.name === name)) {
      errors.push('ConfigMap already exists');
    }
    
    return errors;
  };

  const handleConfigMapNameChange = (name: string) => {
    setNewConfigMap(prev => ({ ...prev, name }));
    setErrors(validateConfigMapName(name));
  };
  ...
  const handleCreateConfigMap = () => {
    const validationErrors = validateConfigMapName(newConfigMap.name);
    if (validationErrors.length > 0) {
      setErrors(validationErrors);
      return;
    }

    if (Object.keys(newConfigMap.data).length === 0) {
  ...
  const handleDeleteConfigMap = (configMapName: string) => {
    onDeleteConfigMap(configMapName);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center px-2 py-1 bg-green-600 text-white rounded text-xs hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
              ...
              <button
                ...
                disabled={!newConfigMap.name || errors.length > 0 || Object.keys(newConfigMap.data).length === 0}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="text-lg font-semibold text-gray-900">Existing ConfigMaps</h4>
                <span className="text-sm text-gray-500">{configMaps.length} total</span>
              </div>
                  ...
                  <div key={configMap.name} className="bg-white rounded-lg p-4 border border-gray-200">
                      ...
                      <div>
                        {deleteConfirm === configMap.name ? (
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              onClick={() => handleDeleteConfigMap(configMap.name)}
                          ...
                          <button
                            onClick={() => setDeleteConfirm(configMap.name)}
                    ...
                    {deleteConfirm === configMap.name && (
                      <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded text-xs text-red-700">
                        ...
                        <div>
                          This will remove the ConfigMap and any references to it in deployments.
                        </div>
                      </div>
                    )}
                  </div>
