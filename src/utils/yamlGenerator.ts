import type { DeploymentConfig, DaemonSetConfig, KubernetesResource, Namespace, ConfigMap, Secret, ServiceAccount, ProjectSettings, JobConfig, CronJobConfig, Container, EnvVar, DockerHubSecret, KubernetesRole, KubernetesClusterRole, PolicyRule, CustomResourceDefinition } from '../types';
...
data:
  ...
  // Add header comment
  if (deployments.length > 0 || daemonSets.length > 0 || customNamespaces.length > 0 || configMaps.length > 0 || secrets.length > 0 || dockerHubSecrets.length > 0 || jobs.length > 0 || cronjobs.length > 0 || serviceAccounts.length > 0 || roles.length > 0 || clusterRoles.length > 0 || roleBindings.length > 0 || crds.length > 0) {
